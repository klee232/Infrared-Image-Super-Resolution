...
class Thermal:
    def __init__(self,
                 ...
                 subset='train',
                 downgrade='bicubic',
                 images_dir='/mnt/d/data',
                 caches_dir='/mnt/d/data/caches'):
        ...
        if subset == 'train':
            self.image_ids = range(1, 7969)
        elif subset == 'valid':
            self.image_ids = range(801, 1360)
        else:
    ...
    def _hr_cache_file(self):
        return os.path.join(self.caches_dir, f'Thermal_{self.subset}_HR.cache')

    def _lr_cache_file(self):
        return os.path.join(self.caches_dir, f'Thermal_{self.subset}_LR_{self.downgrade}_X{self.scale}.cache')
    ...
    def _hr_images_dir(self):
        return os.path.join(self.images_dir, f'Thermal_{self.subset}_HR')

    def _lr_images_dir(self):
        if self._ntire_2018:
            return os.path.join(self.images_dir, f'Thermal_{self.subset}_LR_{self.downgrade}')
        else:
            return os.path.join(self.images_dir, f'Thermal_{self.subset}_LR_{self.downgrade}', f'X{self.scale}')

    def _hr_images_archive(self):
        return f'Thermal_{self.subset}_HR.zip'

    def _lr_images_archive(self):
        if self._ntire_2018:
            return f'Thermal_{self.subset}_LR_{self.downgrade}.zip'
        else:
            return f'Thermal_{self.subset}_LR_{self.downgrade}_X{self.scale}.zip'
